<?
	...
	function true_week_send($ready_db, $group_uniq, $T_Monday, $T_Tuesday, $T_Wednesday, $T_Thursday, $T_Friday, $T_Saturday)
	{
		try
		{
			...
			{
				query_send($ready_db, 'TMonday', '4');
			}

			if($T_Tuesday == "TdisableChekbox")
			{
				query_send($ready_db, 'TTuesday', '6');
			}

			if($T_Wednesday == "TdisableChekbox")
			{
				query_send($ready_db, 'TWednesday', '8');
			}

			if($T_Thursday == "TdisableChekbox")
			{
				query_send($ready_db, 'TThursday', '10');
			}

			if($T_Friday == "TdisableChekbox")
			{
				query_send($ready_db, 'TFriday', '12');
			}

			if($T_Saturday == "TdisableChekbox")
			{
				query_send($ready_db, 'TSaturday', '14');
			}
		}
		catch (Exception $e)
		{
	...
	function false_week_send($ready_db, $group_uniq, $F_Monday, $F_Tuesday, $F_Wednesday, $F_Thursday, $F_Friday, $F_Saturday)
	{
		...
		try
		{
			if($F_Monday == "FdisableChekbox")
			{
				query_send($ready_db, 'FMonday', '3');
			}

			if($F_Tuesday == "FdisableChekbox")
			{
				query_send($ready_db, 'FTuesday', '5');
			}

			if($F_Wednesday == "FdisableChekbox")
			{
				query_send($ready_db, 'FWednesday', '7');
			}

			if($F_Thursday == "FdisableChekbox")
			{
				query_send($ready_db, 'FThursday', '9');
			}

			if($F_Friday == "FdisableChekbox")
			{
				query_send($ready_db, 'FFriday', '11');
			}

			if($F_Saturday == "FdisableChekbox")
			{
				query_send($ready_db, 'FSaturday', '13');
			}
		}
		catch (Exception $e)
		{
